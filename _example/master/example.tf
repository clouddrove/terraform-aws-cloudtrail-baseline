...
data "aws_iam_policy_document" "cloudtrail_key_policy" {
  ...
  statement {
    ...
    principals {
      type        = "AWS"
      identifiers = [format("arn:aws:iam::%s:root", data.aws_caller_identity.current.account_id), "arn:aws:iam::946010253026:root"]
    }
    actions   = ["kms:*"]
    resources = ["*"]
  }
  ...
  statement {
    ...
    condition {
      ...
      values = [
      format("arn:aws:cloudtrail:*:%s:trail/*", data.aws_caller_identity.current.account_id),"arn:aws:cloudtrail:*:946010253026:trail/*"]
    }
  }
  ...
  statement {
    ...
    condition {
      test     = "StringEquals"
      variable = "kms:CallerAccount"
      values   = [data.aws_caller_identity.current.account_id,946010253026]
    }
  }
  statement {
    ...
    condition {
      test     = "StringLike"
      variable = "kms:EncryptionContext:aws:cloudtrail:arn"
      values   = [format("arn:aws:cloudtrail:*:%s:trail/*", data.aws_caller_identity.current.account_id),"arn:aws:cloudtrail:*:946010253026:trail/*"]
    }
  }
